...
def signup(user_accounts, log_in, username, password):
    ...
    '''
    # your code here
    if username not in user_accounts and username != password:
...
def valid_password(password):
    ...
    if len(password) > 7 and no > 0 and a > 0 and b > 0:
        return True
    else:
        return False
...
def import_and_create_accounts(filename):
    ...
    for line in lines:
        ...
        if len(lst) <= 1:
            ...
            signup(user_accounts, log_in, lst[0].strip(), lst[1].strip())
    f.close()
...
def login(user_accounts, log_in, username, password):
    ...
    if username in user_accounts:
        if password == user_accounts.get(username):
...
def change_password(user_accounts, log_in, username, old_password, new_password):
    ...
    if username in user_accounts and log_in[username] == True:
        if old_password == user_accounts[username] and new_password != user_accounts[username]:
